...
from .base.split_wrapper import SplitWrapper

# from utils import voxel_coords_to_world_coords
import logging
import os
from typing import Dict
import numpy as np
    ...
    def create_all_filelist(self):
        ...
        for t in range(self.start_timestep, self.end_timestep):
            for cam_idx in self.camera_list:
                ...
                if self.pixel_data_config.load_disp:
                    disp_filepaths.append(
                        os.path.join(self.data_path, "disp", f"{t:03d}_{cam_idx}.npy")
                    )
        ...
        self.disp_filepaths = np.array(disp_filepaths)
...
class WaymoDataset(SceneDataset):
    ...
    def __init__(
        ...
        self.data_path = os.path.join(self.data_cfg.data_root, f"{data_cfg.scene_idx:03d}")
        ...
        # 每个nerf负责20 frames
        self.num_models = len(self.train_indices)//data_cfg.num_frame_per_nerf if len(self.train_indices)%data_cfg.num_frame_per_nerf==0 else len(self.train_indices)//data_cfg.num_frame_per_nerf+1
        # if cfg_pose.learn_poses and not cfg_pose.use_gt_poses:
        #     self.pose_param_net = LearnPose(len(self.train_indices), cfg_pose.learn_R, 
        #                     cfg_pose.learn_t, init_c2w=None).to(device=device)
        # else:
        #     self.pose_param_net = None
    ...
    def build_split_wrapper(self):
        ...
        if self.pixel_source is not None:
            train_pixel_set = SplitWrapper(
                ...
                num_frame_per_nerf=self.data_cfg.num_frame_per_nerf,
                test_image_start_id=self.data_cfg.pixel_source.test_image_start_id,
            )
        ...
        if self.lidar_source is not None:
            ...
            full_lidar_set = SplitWrapper(
                ...
                num_frame_per_nerf=self.data_cfg.num_frame_per_nerf,
                test_image_start_id=self.data_cfg.pixel_source.test_image_start_id,
                # pose_param_net=self.pose_param_net, # 没有划分训练集和测试集，暂时这里也是用学出来的pose
            )
            if len(self.test_indices) > 0:
    ...
    def build_data_source(self):
        ...
        load_pixel = (
            ...
            or self.data_cfg.pixel_source.load_disp
        )
        if load_pixel:
    ...
    def split_train_test(self):
        if self.data_cfg.pixel_source.test_image_stride != 0:
            test_timesteps = np.arange(
                ...
                int(self.data_cfg.pixel_source.test_image_stride/2),
                self.num_img_timesteps,
                self.data_cfg.pixel_source.test_image_stride,
            )
            # assert len(test_timesteps) == int(self.num_img_timesteps/self.data_cfg.num_frame_per_nerf + 1), "Error in test length"
        else:
            test_timesteps = []
    ...
    def get_occ(self, index: int):
        ...
        return occ_labels, normed_timestamps
