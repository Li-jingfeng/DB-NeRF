    ...
    def sample_uniform_rays(
        ...
        if candidate_indices is None:
            ...
            if not isinstance(candidate_indices, Tensor):
                ...
                # if torch.numel(candidate_indices)==1:
                #     candidate_indices = candidate_indices.repeat(num_rays)
                ...
                if self.cached_indices is None:
                    ...
                    if torch.numel(candidate_indices)==1: # 一张图一张图的学
                        mask |= (
                            self.timesteps == candidate_indices.item()
                        )
                    else:# vanilla
            ...
            if not torch.equal(candidate_indices, self.cached_indices):
                ...
                if torch.numel(self.cached_indices)==1: # 一张图一张图的学
                    mask |= (
                        self.timesteps == candidate_indices.item()
                    )
                else:
