...
class ScenePixelSource(abc.ABC):
    ...
    def to(self, device: torch.device) -> "ScenePixelSource":
        ...
        if self.data_cfg.load_disp:
            self.disps = self.disps.to(device)
    ...
    def load_disp(self) -> None:
        """
        Load the DPT predict disp if they are available.
        """
        if not self.data_cfg.load_disp:
            return
        disps = []
        for frame in tqdm(
            self.disp_filepaths, desc="Loading odom predict disp(1/depth)", dynamic_ncols=True
        ):
            disp = np.load(frame)
            disp = cv2.resize(disp, (self.data_cfg.load_size[1], self.data_cfg.load_size[0]), interpolation=cv2.INTER_AREA)
            disps.append(disp)
        self.disps = torch.from_numpy(np.stack(disps, axis=0)).float()
    ...
    def sample_important_rays(
        ...
        if torch.numel(img_candidate_indices)==1: # 一张图一张图的学
            img_idx = img_candidate_indices.repeat(num_rays)
        else:
    ...
    def sample_uniform_rays(
        ...
        if torch.numel(img_candidate_indices)==1:
            img_id = img_candidate_indices.repeat(num_rays) # 同一张图重复这么多次
        else: #img_candidate_indices不是标量
    ...
    def get_train_rays(
        ...
        """
        rgb, sky_mask, dynamic_mask, odom_disp, features = None, None, None, None, None
        ...
        if self.data_cfg.load_disp:
            odom_disp = self.disps[img_idx, y, x] 
        ...
        if self.data_cfg.load_disp:
            tmp_dict={"odom_depth": 1/odom_disp,"odom_disp": odom_disp}
        data = {
            "x": x,
            "y": y,
    ...
    def get_render_rays(self, img_idx: int) -> Dict[str, Tensor]:
        ...
        data = {
            ...
            "y": y,
            "intrinsics": intrinsics,
            "render_img_width": img_width,
            "render_img_height": img_height,
            "origins": origins,
            "viewdirs": viewdirs,
