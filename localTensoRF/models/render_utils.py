...
from .radiance_field import DensityField, RadianceField
...
def render_rays(
    ...
    if len(rays_shape) == 3:
        ...
        for k, v in data_dict.items():
            if k not in ['x', 'y', 'intrinsics', 'render_img_height', 'render_img_width']:
    ...
    def query_fn(t_starts, t_ends):
        ...
        sub_dict = {
            k: v[..., None].repeat_interleave(t_starts.shape[-1], dim=-1)
            if v is not None else None for k, v in chunk_data_dict.items()
    ...
    for i in range(0, num_rays, chunk):
        chunk_data_dict = {k: v[i : i + chunk] if v is not None else None for k, v in reshaped_data_dict.items()}
