...
from .nerf_utils import contract, find_topk_nearby_timesteps, trunc_exp
from .mlp import MLP
    ...
    def forward(
        ...
        if self.dynamic_xyz_encoder is not None and has_timestamps:
            ...
            if "normed_timestamps" in data_dict:
                data_dict["normed_timestamps"] = data_dict["normed_timestamps"].to(normed_positions.device)
                ...
                data_dict["lidar_normed_timestamps"] = data_dict["lidar_normed_timestamps"].to(normed_positions.device)
        ...
        if (
            ...
            reduced_data_dict = {k: v[:, 0] if v is not None else None for k, v in data_dict.items()}
