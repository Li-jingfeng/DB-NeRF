...
class ViTExtractor:
    ...
    def create_model(model_type: str) -> nn.Module:
        ...
        return model
    # 因为没办法直接用torch.hub，所以这里需要手动下载dinov2以及保存权重
    # @staticmethod
    ...
    # @staticmethod
    # def create_model(model_type: str, local_weight_path: str = None) -> nn.Module:
    #     if "dinov2" in model_type:
    #         # 创建模型实例
    #         model = MyModel()
    #         if local_weight_path:
    #             # 加载本地权重
    #             MyModel.load_weights(model, local_weight_path)
    #         else:
    #             raise ValueError("Local weight path is required for Dinov2 model.")
    #     # 不支持dino
    #     else:
    #         raise NotImplementedError("Only dino and timm models are supported at the moment.")
    #     return model
